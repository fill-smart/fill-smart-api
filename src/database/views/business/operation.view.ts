...
import { PaymentMethodsEnum } from "../../models/business/purchase.model";
...
NULL AS gas_station_name, customer.first_name AS customer_first_name, customer.last_name AS customer_last_name, 
customer.document_number AS customer_document_number, 
NULL AS fuel_price, transfer.target_litres AS litres, 
6 AS operation_type_id, 'Transferir litros' AS operation_type_name, 
NULL AS pump_external_id, NULL AS authorization_status, NULL AS exchange_source_fuel_type, 
user.id AS user_id, NULL AS payment_method,
NULL AS total,
...
LPAD(transfer.id, 8, '0') AS transaction_id
  ...
  LEFT JOIN wallet wallet ON transfer.source_wallet_id = wallet.id
  LEFT JOIN customer customer ON wallet.customer_id = customer.id
  LEFT JOIN fuel_type fuel_type ON wallet.fuel_type_id = fuel_type.id
  LEFT JOIN user user ON customer.user_id = user.id
  LEFT JOIN wallet target_wallet ON transfer.target_wallet_id = target_wallet.id
